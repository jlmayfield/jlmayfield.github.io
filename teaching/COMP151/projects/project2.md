...
* Have a well defined main function that coordinates the following functionality:
    * Create 8 beats of music at 200 bpm using the following note sequence: A3, C4, E4, G4, A4, G4, E4, C4. Each note should last for half a beat and occur at the beginning of each beat. You can choose whichever wave form and amplitude you'd like.  
...
This pattern is the basis for what is called the *chromatic scale*. It actually doesn't matter where you start. If you start at a G and call its frequency f, then the 12 note sequence up to the next G follows this same pattern as moving from C to C. Where this becomes useful is in mixing in related notes to the dominant tone of a voice. Let's say your voice is meant to produce some note with frequency *f* (maybe 440). You might like to mix in just a little bit the fourth or fifth above that note. These notes tend to be very resonant with your dominant frequency and work well in combination with that note. This is the equivalent of F and G on the chart above. You can just take your dominant frequency *f* and multiply by (4/3) or (3/2) to get the right note for you mix.

#### Sound Effects

You'll be implementing program versions of little bit synth kit components. Below you'll find descriptions of each component as a function acting on an with sound data.
...
For this project you're welcome to find sounds you like through trial and error and now worry about the overall proportions. If you find that you have too much or too little of a sound in a mix containing more than two sounds, then keep this discussion in mind.

#### Envelope

Natural sounds do not typically start at one volume and stay at the volume for as long as you'd like them to, but this is the kind of sound our wave generators and voices produce. Natural sounds typically have a volume *envelope* where the volume grows to a peak and then shrinks to nothing. The growth phase is called the *attack*. The shrinking phase is called the *decay*. Basic envelope effects let you set the length of attack and decay.

Our *envelope* function should take a sound, an attack length in seconds, and a decay length in seconds. It should then increase the amplitude from 0% to 100% for the attack phase and 100% back to 0% for the decay phase. The catch is that the input sound does not need to be as long as the sum of the attack and decay time. Imagine banging a gong. It has a short attack and a long decay. If you hit it once it and leave it alone you'll hear a loud tone that slowly decays to nothing. If you hit it again before it stops ringing, then you are effectively stopping the old envelope mid decay and starting a new envelope. Our *envelope* function should support this kind of musical sound by computing as much of the envelope as fits the given sound. Suppose your envelope has an attack of 0.25 seconds and a decay of 1 second. If the given sound is 0.5 seconds, then you get all of the attack phase and only 0.25 seconds of decay. If the given sound is 2 seconds you get the full envelope including 0.75 seconds of silence created at the end of the sound.
